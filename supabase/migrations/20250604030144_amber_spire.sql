...
CREATE INDEX idx_system_health_name_ts ON public.system_health_metrics USING btree (metric_name, ts);
CREATE INDEX idx_system_health_ts ON public.system_health_metrics USING btree (ts);
...
CREATE INDEX idx_model_metrics_model_timestamp ON public.model_performance_metrics USING btree (model_id, timestamp);
CREATE INDEX idx_model_metrics_timestamp ON public.model_performance_metrics USING btree (timestamp);
...
CREATE INDEX idx_client_journeys_started ON public.client_journeys USING btree (started_at);
CREATE INDEX idx_client_journeys_user_started ON public.client_journeys USING btree (user_id, started_at);

-- Fix security_alerts indexes
DROP INDEX IF EXISTS idx_unresolved_alerts;
CREATE INDEX idx_unresolved_alerts ON public.security_alerts USING btree (resolved, timestamp) WHERE (NOT resolved);
