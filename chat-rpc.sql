-- Chat RPC Function for Genesis Web App
...
BEGIN
    ...
    -- Track AI usage
    PERFORM track_ai_usage(user_uuid, 50); -- Estimate 50 tokens per response
...
BEGIN
    RETURN (
        SELECT jsonb_agg(
            jsonb_build_object(
                'conversation_id', ach.conversation_id,
                'last_message', ach.message,
                'last_updated', ach.created_at,
                'message_count', (
                    SELECT COUNT(*) 
